...
static void BM_histogram_add_mask(benchmark::State &state) {
    const auto trace   = TraceLog("harness.bundle");
    const auto regions = trace.macho_regions();
    std::vector<uint32_t> instrs;
    instrs.resize(trace.num_inst());
    ...
    i = 0;
    for (auto _ : state) {
        hist.add_mask(instrs_buf[i % num_instrs]);
        ++i;
    }
}

// BENCHMARK(BM_histogram_add_mask);

static void BM_histogram_add_hash(benchmark::State &state) {
    const auto trace   = TraceLog("harness.bundle");
    const auto regions = trace.macho_regions();
    std::vector<uint32_t> instrs;
    instrs.resize(trace.num_inst());
    ...
    i = 0;
    for (auto _ : state) {
        hist.add_hash(instrs_buf[i % num_instrs]);
        ++i;
    }
}

// BENCHMARK(BM_histogram_add_hash);

static void BM_histogram_add_lut(benchmark::State &state) {
    const auto trace   = TraceLog("harness.bundle");
    const auto regions = trace.macho_regions();
    std::vector<uint32_t> instrs;
    instrs.resize(trace.num_inst());
    ...
    i = 0;
    for (auto _ : state) {
        hist.add_lut(instrs_buf[i % num_instrs]);
        ++i;
    }
}

BENCHMARK(BM_histogram_add_lut);
