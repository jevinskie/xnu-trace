...
TEST_CASE("exact_fit", TS) {
    const auto ebv = BitVector<16, false>{16, 0};
    REQUIRE(ebv.exact_fit(16));
    const auto ebv2 = BitVector<16, false>{8, 0};
    REQUIRE(ebv.exact_fit(8));
    const auto ebv3 = BitVector<32, false>{16, 0};
    REQUIRE(ebv.exact_fit(16));
    const auto nebv = BitVector<16, false>{15, 0};
    REQUIRE_FALSE(nebv.exact_fit(15));
    const auto nebv2 = BitVector<16, false>{15, 0};
    REQUIRE_FALSE(nebv2.exact_fit(7));
    const auto nebv3 = BitVector<32, false>{24, 0};
    REQUIRE_FALSE(nebv3.exact_fit(24));
    const auto nebv4 = BitVector<32, false>{4, 0};
...
TEST_CASE("buf_size", TS) {
    ...
    const auto bv = BitVector<16, false>{15, 0};
    REQUIRE(bv.buf_size(15, 3) == 3);
    const auto abv = AtomicBitVector<16, false>{15, 0};
